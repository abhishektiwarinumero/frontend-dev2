...
export default {
	...
	computed: {
		selectedrankID: {
			get() {
				return this.$store.state.valorant.rank;
			},
			set(id) {
				this.$store.commit("valorant/changerank", id);
				this.selectedDivisionID = this.rank.divisions[2].id;
			},
		},
		selectedDivisionID: {
			get() {
				return this.$store.state.valorant.division;
			},
			set(id) {
				this.$store.commit("valorant/changeDivision", id);
			},
		},
		rank() {
			let index = this.$store.state.valorant.rank;
